...
export function WebsiteRender({
  ...
  return uiElements.map((uiElement) => {
    ...
    if (uiElement.element.type == "Container") {
      const direction =
        uiElement.element.direction == "row" ? "flex-row" : "flex-col";
      const justify = `justify-${uiElement.element.justify}`;
      const align = `items-${uiElement.element.align}`;
      styles = {
        ...styles,
        backgroundColor: uiElement.element.backgroundColor,
        "border-width": `${uiElement.element.borderWidthPx}px`,
      };
      return (
      ...
      styles = {
        fontSize: `${uiElement.element.fontSizeRem}rem`,
        color: `${uiElement.element.color}`,
      };
      return (
        <span style={styles}>
          {uiElement.element.url == null ? (
            uiElement.element.value
          ) : (
            <a href={uiElement.element.url} target="_blank">
              {uiElement.element.value}
            </a>
          )}
        </span>
      ...
      return <Button style={styles}>{uiElement.element.text}</Button>;
      ...
      return (
        <img
          height={
            uiElement.element.url == null
              ? undefined
              : uiElement.element.heightPx
          }
          width={
            uiElement.element.url == null
              ? undefined
              : uiElement.element.widthPx
          }
          src={
            uiElement.element.url == null
              ? `https://placehold.co/${uiElement.element.widthPx}x${uiElement.element.heightPx}`
              : uiElement.element.url
          }
        />
      );
    }
